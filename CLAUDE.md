...
- Не додаємо WYSIWYG-редактор, окрім одного поля: опис для Prom — це HTML, і його редагує
  міні-редактор Tiptap з режимом сирого HTML, а `api` чистить його перед записом
  ([ADR 0016](docs/features/product-creation-flow/adr/0016-store-the-prom-description-as-html.md)).
  Решта текстів редагується в `textarea` як plain text.
