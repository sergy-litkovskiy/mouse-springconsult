...
- Модель — `claude-sonnet-5` для всіх викликів ([ADR 0004](../../../../../docs/adr/0004-use-sonnet-5-for-card-preparation.md)).
  Модель, `effort` і `max_uses` пошуку — константи `src/config.ts` **окремо на кожен виклик**
...
- **Розробка майже не витрачає API.** Тести й локальний стек за замовчуванням працюють
  з фейковим адаптером або записаною відповіддю; ключа немає ні в тестах, ні в CI. Ключ
  розробки — з окремого workspace Console з лімітом $10 на місяць. Живий виклик —
  свідома дія, серія прогонів оцінюється в доларах до запуску.
- Кожен виклик записує `usage` (`input_tokens`, `output_tokens`) і `model` у свій рядок
  `product_preparation_runs` — без цього неможливо рахувати собівартість картки. Окремої
  таблиці `ai_generations` немає: вартість картки — сума по запусках.
