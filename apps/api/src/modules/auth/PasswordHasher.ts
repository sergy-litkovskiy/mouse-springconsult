 ...
 * first sign-in would cost a different amount of time and give itself away.
 */
const hashOptions = {
...
export class PasswordHasher {
  /**
   * Hash of a password that does not exist: verifying against it for an unknown email levels the
   * response time, so brute force cannot tell "no such user" from "wrong password". It is a real
   * argon2id hash with the working parameters — a cheaper one would show up in the timing.
   */
  readonly decoyHash =
