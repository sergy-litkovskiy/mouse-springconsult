...
/**
 * A file at the `app/` level, not inside `auth`: the rule names a route of the `products`
 * feature, and a guard, an interceptor and the sign-in form all ask it the same question. Left
 * in `auth`, renaming the landing page would mean editing the auth feature — exactly the
 * coupling rule 12 exists to prevent, and one ESLint cannot see, because it is a string rather
 * than an import.
 */
...
/**
 * The shape of a `returnUrl` can be right while its contents are unreadable: `/products?q=100%`
 * is such a URL — a lone `%` is not an escape sequence — and Angular's serialiser does not reject
 * it, it returns the empty tree, which would silently discard the destination the admin asked
 * for. The `catch` covers a serialiser that raises instead of collapsing, which is what
 * `decodeURIComponent` does on its own.
 */
