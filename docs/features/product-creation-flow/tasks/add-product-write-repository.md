...
**AC-04** (US-02) — happy path, **US-02/AC-04 retired 2026-09-12** ([ADR 0014](../adr/0014-let-ai-recognize-the-item-from-photos.md))
...
Історичний запис: код, який ця задача додала, не змінюється — репозиторій і сьогодні пише
всі поля картки одним оновленням. Змінилась лише історія, якій це служило: розпізнавання
більше не окремий ручний вхід (§4 S6, [sad.md](../sad.md)).
...
- [x] Жодного QueryBuilder поза цим файлом — `deps:check` зелений.
- [x] `update` не перезаписує полів, яких не було в частковому наборі — перевірено тестом на кожному полі, не на одному.
- [x] Ціна повертається тим самим десятковим рядком, який віддав драйвер: ні `transformer` на колонці, ні конвертера тут.
- [x] Каскадне видалення кадрів підтверджено запитом до бази після `delete`.
- [x] `npm run test` в `api` зелений на тестовій базі.
- [x] Коміт: `feat(products): add card write operations to the repository`.
