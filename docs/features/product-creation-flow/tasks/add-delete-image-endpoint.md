...
- [x] Смоук з недосяжним доменом R2: кадр лишається на місці, повтор після відновлення проходить.
  ...
  `critical-path-review` дав WARN: серверна частина AC-16/AC-17 виконана, а три зауваження стосуються клієнта. Їх треба перевірити в [T21](add-gallery-upload-dialog.md): після 204 галерея показується без кадру; 502 `storage_unavailable` показує помилку й лишає кадр; 404 `image_not_found` на повторне видалення не показується як помилка.
