...
- [x] Файл на 10 МБ проходить, на 11 МБ — відхиляється `file_too_large`, а не обривом зʼєднання: перевірено через `caddy`, не лише напряму в `api`: `caddy:2` з цим `Caddyfile` у мережі стека пропустив 10 МіБ (201), а на 11 МБ і на 10 МіБ + 1 байт відповів `file_too_large` від `api`. На 12 МіБ, вище парної межі, Caddy відповідає власним 413.
