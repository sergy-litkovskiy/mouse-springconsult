...
- [x] AC-01: кадр зʼявляється в галереї, відповідь містить складену адресу; публічне читання з R2 — 200 `image/jpeg` (смоук 2026-09-17).
- [x] AC-02: одинадцятий кадр відхиляється кодом `gallery_full`, десять наявних цілі — звірено запитом до бази (10 рядків, один головний, позиції 0–9).
- [x] AC-19: перший кадр порожньої галереї приходить у відповіді з `isMain: true`; другий і наступні — з `isMain: false`, доки їх не призначать явно ([T15](add-set-main-image-endpoint.md)).
- [x] Відмова сховища не лишає рядка в `product_images` — перевірено з недосяжним доменом R2 (QG-1): окремий екземпляр `api` з хибним `R2_ACCOUNT_ID` відповів 502, кількість рядків не змінилась.
- [x] `MediaService` створюється лише в `src/api.ts`; `deps:check` зелений.
- [x] Файл на 10 МБ проходить, на 11 МБ — відхиляється `file_too_large`, а не обривом зʼєднання: перевірено через `caddy`, не лише напряму в `api`: `caddy:2` з цим `Caddyfile` у мережі стека пропустив 10 МіБ (201), а на 10 МіБ + 1 байт, 11 МБ, 11 МіБ і 12 МіБ відповів `file_too_large` від `api`. На 21 МБ, вище парної межі 20 МіБ, Caddy відповідає власним 413.
  Уточнення 2026-09-17: межу файлу тримає `limits.fileSize` плагіна `@fastify/multipart`, а не `bodyLimit` маршруту, бо плагін читає потік сам. Його помилку контролер мапить у `FileTooLarge`. Окремим комітом додано перевірку, що картка існує, ще до звернення до сховища. Без неї кадр для неіснуючої картки лишав би об'єкт у R2 і падав на FK з 500.
  `critical-path-review` (PARTIAL) знайшов три речі; їх виправлено окремим комітом.
  1. `position = count` ламався на галереї з діркою й під паралельними вивантаженнями. Тепер позицію, головний кадр і межу десяти визначає репозиторій у транзакції під блокуванням рядка картки. Тест на Postgres з п'ятьма паралельними вставками падає, якщо блокування прибрати.
  2. Об'єкт, для якого не вдалося записати рядок, сервіс видаляє зі сховища.
  3. Запасу в 1 МіБ між межею файлу й межею проксі бракувало, тож парну межу піднято до 2 × `maxImageBytes`.
- [x] Коміт: `feat(products): accept a gallery frame in a single request`.
