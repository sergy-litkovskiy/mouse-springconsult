...
Чистка вже існує: `cleanDescription` у `apps/api/db/prom-csv.ts` (sanitize-html, CSV-імпорт).
...
Схема не змінюється. Наявні рядки не переписуються: чистка діє лише на запис. Якщо T46
вирішила конвертувати plain-text рядки міграцією, ця міграція належить до цієї задачі.
...
4. Ліміт 8000 символів — за рішенням T46 №6. Перевіряється до чистки, як і зараз, на межі контролера.
5. `openapi.yaml`: у `descriptionProm` обох схем запису дописати, що сервер чистить HTML за переліком ADR 0016. Оновити excerpt цієї story.
6. `PRD.md §5`: AC-46.
