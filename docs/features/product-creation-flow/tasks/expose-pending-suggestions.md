...
- [x] AC-41: картка з ручною правкою віддає пропозицію в `pendingSuggestions`.
- [x] AC-42: автозастосована пропозиція в масиві не з'являється, відхилена — теж.
- [x] Картка без жодного запуску віддає `[]`, а не відсутнє поле.
- [x] Кількість запитів у базу на читання картки не зросла.
- [x] `openapi.yaml` більше не обіцяє трьох полів картки у відповіді каталогу.
- [x] Коміт: `feat(products): answer the card read with its pending suggestions`.
