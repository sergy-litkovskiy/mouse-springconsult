...
**Доопрацювання 2026-09-23, пункт 8:** «після кліку на кнопки "? -> AI" чи "*** -> AI", ці кнопки
мають замінятися на маленький лоадер, поки йде опрацювання запиту. Після появи результата, кнопки
...
**When** `user` тисне «Покращити через AI» чи «Застосувати як промпт» біля цього поля
**Then** поки йде запуск, обидві кнопки запуску цього поля замінює малий `mat-progress-spinner` з `aria-label` «Модель готує варіант», стрілка «Застосувати для поля ліворуч» вимкнена, а в кнопці «Згенерувати все» і в інших полях спінера немає; після результату кнопки повертаються
