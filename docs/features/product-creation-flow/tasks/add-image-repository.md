...
- [x] AC-03: спроба позначити другий кадр головним лишає рівно один головний — перевірено **проти індексу**, а не проти коду.
- [x] Перестановка двох кадрів місцями не падає на унікальному обмеженні — тест на реальній базі, не на двійнику.
- [x] `setMainImage` виконується однією транзакцією; проміжного стану без головного кадру не існує.
- [x] `npm run test` в `api` зелений на тестовій базі.
- [x] Коміт: `feat(products): add gallery frame operations to the repository`.

AC-02 не входить у DoD цієї задачі свідомо (див. Out of scope) — репозиторій дає лічильник,
межу перевіряє сервіс (`T14`). `critical-path-review` підняв це питання під час рев'ю
(бракувало видимості поза diff на Service-шар); override — це не пропуск, а те саме
розмежування, яке DoD уже відображав до рев'ю.
