...
- [x] AC-14: картка після двох запусків показує суму по **обох**, не по останньому. Смоук на живому стеку: два запуски по 1000/200 і 340/55 дали 1340/255.
- [x] Картка без жодного запуску показує нулі, а не `null`. Смоук: `0` і `0` до першого запуску.
- [x] Сума рахується одним запитом, а не циклом у сервісі: `PreparationRepository.sumTokens` — один `coalesce(sum(...), 0)::int` з тестом проти реальної бази.
- [x] Жодної валюти й жодного порогу в коді: у контракті лише два цілих.
- [x] Коміт: `feat(products): expose the card preparation cost in tokens`.

Поля лягли на схему відповіді **читання картки**, а не на спільну `productCardSchema`: нею
типізується й рядок каталогу, а сторінка з 50 рядків рахувала б 50 сум на запит. `openapi.yaml`
досі обіцяє їх на спільній схемі `Product` — розходження закриває [T53](expose-pending-suggestions.md).
