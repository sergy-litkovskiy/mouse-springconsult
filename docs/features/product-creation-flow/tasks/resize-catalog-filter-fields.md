...
- [x] AC-33: на 1280 px вузькі поля ≈ вдвічі вужчі, «Назва» й «Опис» ширші за решту, мітки не обрізані — видно на знімку.
- [x] AC-34: на 360 px горизонтального скролу немає; помилка ціни видна повністю.
- [x] Наявні тести `product-catalog.spec.ts` лишаються зеленими; `lint` у `web` зелений.
- [x] Коміт: `style(web): size the catalog filters by their content`.

Результат 2026-09-17 (для опису PR):
- Вузькі поля — `10.5rem` (168 px), а не `6.5rem`. На 6.5rem і на 9rem Material обрізає плаваючу мітку «Публікація Prom», бо обмежує її ширину відсотком від infix. `min-width: max-content` не підходить: infix має задану ширину, і поле стало б ширшим за колишні 13rem.
- Широкі поля — `2 1 17rem`, а не `20rem`. На 20rem «Категорія» переноситься в другий рядок, розтягується на весь рядок і стає ширшою за «Назву».
- Помилку ціни не скорочено: її текст перевіряє `product-catalog.spec.ts`. Натомість цінові поля мають `subscriptSizing="dynamic"`, і повідомлення переноситься на другий рядок.
- Горизонтальний скрол на 360 px давала шапка застосунку, а не панель: «Вийти» виходила за край і до T37. Виправлено окремим комітом `fix(web)` в `app-layout.css`.
