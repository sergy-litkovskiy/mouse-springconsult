...
- [x] Відмова сховища не видаляє нічого; код `storage_unavailable`.
  ...
  Залишковий компроміс S3 API, який не виправити без зміни підходу: частково вдалий `DeleteObjects` чи втрачена відповідь лишають картку без частини об'єктів. Повторне видалення це дочищає: смоук [T07](add-image-storage-adapter.md) показав, що повторний `deleteMany` на вже видалених ключах у R2 проходить успішно. Повторний `critical-path-review` — WARN, саме з цим зауваженням.
