...
**Then** редактор теж вимкнений у обох режимах, а помилка довжини показується так само, як у `textarea`

## Checklist

1. **Окремим комітом до кроку А** — установити пакет редактора в образ `web` (команда з `mouse-commands`, не на хості).
...
3. `product-form.html`: замінити `textarea` опису Prom на редактор у тому ж `mat-form-field` або поруч з тією самою міткою та `mat-error`. Порожнє поле не дає `<p></p>` у тілі запиту, якщо T46 не вирішила інакше.
4. Наявні plain-text описи відкриваються за рішенням T46 №5.
