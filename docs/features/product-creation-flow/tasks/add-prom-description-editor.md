...
3. `product-form.html`: замінити `textarea` опису Prom на редактор з тією самою міткою. Валідатора довжини в описів уже немає (прибрано в T47, ADR 0016 №6). Порожній редактор дає `""`, а не `<p></p>` (№8). Значення, завантажене з сервера, не позначає форму зміненою, навіть якщо Tiptap його нормалізував.
