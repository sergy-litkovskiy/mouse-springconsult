...
`retryBackoff`, плюс запас на `pollingIntervalSeconds`. За нинішніх значень (три спроби по
300 с і паузи близько 10 і 20 с) це близько 16 хвилин. Запуск `queued`, старший за поріг, теж
...
- [ ] AC-39: запуск `running`, старший за поріг, закривається `failed / preparation_failed` без
  пропозицій. Тест проти реальної бази.
- [ ] AC-40: молодший за поріг `running` і будь-який завершений запуск лишаються незмінними.
  Тест проти реальної бази.
- [ ] Поріг виведено з `config.queue.preparation`, а не записано окремим числом.
- [ ] Смоук: запуск із `created_at` у минулому закривається першим же обходом живого `worker`.
- [ ] Коміт: `fix(ai): close preparation runs left behind by a dead attempt`.
