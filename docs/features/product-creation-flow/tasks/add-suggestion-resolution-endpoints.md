...
Ендпоінти generic-ові за `field`: область запуску, з якої з'явилась пропозиція
(`texts`/`both`/`price`/`field`), тут не має значення — прийняття/відхилення працює
однаково і для пропозицій з фото-розпізнавання, і для пропозицій з регенерації одного поля
([ADR 0015](../adr/0015-add-per-field-text-rewrite-scope.md)); нового коду ця задача не
додає.
