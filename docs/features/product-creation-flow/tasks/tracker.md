...
рамкою поля. Чекає на T69, бо та переносить «Згенерувати все» під галерею й міняє ті самі відступи.
...
Зелений тепер живе в одній змінній `--app-success` у `styles.css`, спільній зі snack bar успіху.
RED довелося виправити amend-ом: наявний тест перевіряв текст `Опубліковано`, який AC-58
прибирає. `pw` на обох станах: сіра іконка має контраст 4.27:1 на звичайному рядку й 3.85:1 на
підсвіченому. Розблокувала T65.

2026-09-25 закрито [T62](pick-catalog-categories-with-autocomplete.md): фільтр «Категорія» —
chips з автодоповненням із `GET /products/categories`, кілька однойменних `category` в URL. `pw`
знайшов регресію поза тестами RED: з двома категоріями в URL живий пошук, пагінація й сортування
стирали незастосовані chips і текст панелі, бо роутер щоразу віддає новий масив. Її й ліміт у 20
chips (межа контракту, категорій у базі 63) закрито окремим комітом з тестами. Нічого не
розблокувала: у T62 немає залежних.
...
(порожнє слово після редагування, повтор після редагування, текст, втрачений без `Enter`), їх
закрито окремим комітом з тестами. Нічого не розблокувала: T62 ще чекає на T60, T66 — на T73.
...
Запит 2026-09-21 після приймання поставки 2; T67–T74 додано 2026-09-23, T75–T77 — 2026-09-25. Ланцюжки в `blocked_by` — здебільшого спільні файли
