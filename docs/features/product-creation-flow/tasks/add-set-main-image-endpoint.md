...
- [x] AC-03: після виклику головним лишається рівно один кадр — перевірено запитом до бази, не лише відповіддю: тест репозиторію на Postgres і смоук 2026-09-17 (`count(*) filter (where is_main)` = 1).
- [x] Кадр чужої картки дає `image_not_found`, а не мовчазний успіх.
- [x] Повторний виклик з тим самим кадром не падає — ідемпотентність `PUT` підтверджена тестом.
- [x] Маршрут під `sessionGuard`: тест контролера з guard, що відмовляє, і смоук без куки → 401.
- [x] Коміт: `feat(products): add the main-frame endpoint`.
