...
> **Вихід:** цей файл · [tracker.md](tracker.md) · 37 story у цій теці · [CONTEXT.md](../CONTEXT.md) фічі.
...
| **Поставка 3** | Запит 2026-09-21 після приймання поставки 2: десять правок UI каталогу й форми картки. Пункт 6 (кілька категорій) розрізано на `api` і фронт. Quality gate поставок 1–2 не змінює й від них не залежить. T67 (2026-09-23) — назви Prom/OLX у «Згенерувати все» | T56–T67 | ~8 д |
...
поставка 3
рівень  0 │ T56  T57  T58  T59  T60  T61  T63  T64  T67
рівень  1 │ T62  T65  T66
```

**Поставка 3 стоїть на рівнях 0–1** не тому, що йде разом із поставкою 1: її задачі не
залежать від жодної відкритої, а все, на що вони спираються в коді, уже змерджено. Усередині
поставки рівень 0 — дев'ять незалежних задач, але спільні файли (`product-catalog.*`,
...
з усією нею. П'ятнадцять рівнів — це і є критичний шлях у 15 задач (T50 додано 2026-09-19).

**Граф ациклічний.** Перевірено топологічним сортуванням: усі 49 вершин потрапляють у
...
**Фактичний стан (2026-09-16):** 37 з 37 проходять усі вісім — 296 перевірок, нуль
...
максимум 3800, середнє 1808 — стеля в 5000 має запас на excerpt-и, які виконавець
підтягне за лінками.
...
Нові AC-50…AC-61 кожна story вносить у PRD §5 сама; T67 ще й переписує AC-05.
...
| [T66](show-local-ai-progress.md) | Локальний індикатор запиту до AI біля кнопки й поля | T58, T59 | S | Serhii |
...
|---|---|---|---|
| 1 | Іменний власник на задачу, `<TBD lead>` для відкритих | Усі 37 задач — Serhii, плейсхолдера немає | PRD §3, `CONTEXT.md`: у системі один-два адміни, ролей немає. Розподіл власників не має предмета |
