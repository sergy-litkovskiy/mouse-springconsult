...
Розпізнавання речі за фото спочатку мало лишитись ручним кроком (idea-brief §13 назвав
вузьким місцем кроки 2-4, а не крок 1), але макет екрана картки (2026-09-12) зробив і цей
крок автоматичним: `worker` розпізнає товар із головного кадру як частину виклику, що готує
тексти ([ADR 0014](adr/0014-let-ai-recognize-the-item-from-photos.md)); `user`, який цьому
не довіряє, вписує тексти сам (US-06) — обидва шляхи рівноправні. Кожне окреме поле також
можна перегенерувати з уже написаного людиною тексту, без фото
([ADR 0015](adr/0015-add-per-field-text-rewrite-scope.md)).
...
- **Поставка 1 — картка й галерея.** Створення й редагування картки, галерея до десяти
  ...
  Покриває US-01, US-05 у частині ручного редагування, US-06, US-07.
- **Поставка 2 — модель.** Тексти під обидва майданчики (з розпізнаванням товару за
  головним кадром, [ADR 0014](adr/0014-let-ai-recognize-the-item-from-photos.md)), діапазон
  ціни, регенерація окремого поля з чернетки без фото ([ADR 0015](adr/0015-add-per-field-text-rewrite-scope.md)),
  збереження ручних правок при повторному запуску, вартість картки, а разом з ними — черга,
  процес `worker` і `sharp` як оптимізація кадру перед викликом. Покриває US-03, US-04,
  US-05 повністю, US-08, US-10; US-02 закрито разом з переходом на розпізнавання за фото.
...
- PostgreSQL 18 + TypeORM 1.1; `typeorm` і `pg` згадуються тільки в `*Repository.ts` і в
  entity-класах. Перелік entity в `.dependency-cruiser.cjs` іменний — нова entity
  дописується туди явно, і це навмисно:
...
- **Модель.** `claude-sonnet-5` ([ADR 0004](../../adr/0004-use-sonnet-5-for-card-preparation.md)) — константа `src/config.ts`, не env-змінна: заміна моделі
  ...
  Вимогу idea-brief §1 «безкоштовно або мінімальний cost» закрив ADR 0004: оцінка —
  ≈ $0.08–0.10 на картку, 100 карток у межах $10 на місяць; реальні цифри дасть `usage`
  (рядок у §11).
...
| Anthropic API | System (external, поставка 2) | `worker` викликає `claude-sonnet-5` зі structured outputs — тексти під обидва майданчики; окремим викликом `web_search_20260209` з локацією UA — діапазон ринкових цін |
...
### S6. Розпізнавання товару — вхід моделі, а не поле картки

Товар розпізнається з до трьох кадрів галереї в тому самому виклику, що готує тексти:
результат ніде не персистується, а йде прямо в структуровану відповідь моделі. Це замінює
поставлену раніше вимогу «`user` вписує розпізнавання сам, перш ніж запускати підготовку» —
кнопка «Generate all» тепер робить обидва кроки одним запитом за бажанням `user`-а, а ручний
шлях (US-06) лишається незалежним і рівноправним. Наслідок для гейту AC-06: він перевіряє
лише наявність кадру, «не внесено розпізнавання» більше не причина відмови.
Деталі: [ADR 0014](adr/0014-let-ai-recognize-the-item-from-photos.md).

### S7. Окреме поле можна перегенерувати з його чернетки, без фото

Поруч із групованими запусками (`texts`/`price`/`both`) з'являється четверта область —
`field`: один виклик моделі отримує лише те, що `user` уже написав у конкретному полі, і
повертає новий варіант того самого поля, нічого не чіпаючи в решті картки. Вхід — чернетка
з тіла запиту, а не збережене значення `products`, бо `user` міг ще не натиснути
«Зберегти». Пропозиція лягає в ту саму таблицю `product_field_suggestions`, а прийняття йде
тим самим `acceptFieldSuggestion`, яким уже приймаються пропозиції з фото-запуску — нового
UI-патерну чи ендпоінту ця область не додає.
Деталі: [ADR 0015](adr/0015-add-per-field-text-rewrite-scope.md).

**Уточнення 2026-09-13 (кнопка ціни, PRD AC-23–AC-27).** Пошук ціни (`scope: price`, §4 S3)
отримує власну кнопку «? -> AI» між лівим і правим полем ціни — доступну будь-коли, поки в
картці є заголовок (`titleProm` або `titleOlx`; AC-24, AC-27), а не лише як шлях відновлення
після часткової відмови (AC-10b, сценарій 5). Вхід пошуку — заголовок (Prom пріоритетно,
інакше OLX) і, якщо він є, опис (та сама пріоритетність); сам по собі опис без заголовка
входом не є — деталі формули в §6, сценарій 8. На відміну від текстових полів (S7), у ціни
немає «<- AI»: причина не в дизайні, а в типі даних. `product_field_suggestions.value` для
`field: price` — діапазон `{priceFrom, priceTo}` ([data-model.md](data-model.md)), а колонка
`products.price` — скаляр, тож писати діапазон у неї generic-ом `acceptFieldSuggestion` (T30)
неможливо фізично, не лише за домовленістю. Тому `acceptFieldSuggestion` explicit-но виключає
`field: price` (AC-26, §11); праворуч — лише текст-довідка, жодного кліку чи автозаповнення.
Число в ліве поле вписує сам `user` — орієнтуючись на показаний діапазон або власним
рахунком, тим самим шляхом, що й ручне введення — сценарієм 6, через звичайне збереження
картки (AC-09, AC-12, AC-25). Нового ендпоінту ця кнопка не додає: запуск — той самий
`POST /preparation-runs` з `scope: price`, який уже існував для AC-08.

### S8. Опис для Prom — HTML, який `api` чистить перед записом

Уточнення 2026-09-18, [T46](tasks/decide-prom-description-html.md). `description_prom`
зберігає HTML з короткого переліку тегів, а не plain text. Поле редагує міні-редактор
Tiptap з режимом сирого HTML, а кнопка «Почистити html» чистить його в браузері через
DOMPurify. Межею безпеки лишається `api`: під час створення й оновлення картки він чистить
опис тим самим переліком (`sanitize-html`), тож прямий запит в обхід форми збереженого XSS
не дасть. Схема не змінюється, змінюється зміст колонки. Опис для OLX і відповідь моделі
лишаються plain text, а в `<p>` відповідь моделі перетворюється під час прийняття
пропозиції. Деталі: [ADR 0016](adr/0016-store-the-prom-description-as-html.md).

---

Кожне тактичне рішення пізніших секцій зводиться до одного з цих восьми стовпів. Рішення, яке
...
apps/web/src/app/products/form/       product-form.ts/html/css — нова підфіча,
                                      відкривається як MatDialog з каталогу (створення й
                                      редагування — той самий діалог); складає в собі
                                      секцію gallery/ (нижче) і, з поставки 2, панель
                                      генерації текстів (§6, макет картки)
...
**Уточнення 2026-09-12 (макет картки, `docs/assets/add-edit-product-feature.pdf`).**
Картка товару відкривається одним попапом, а не переходом на маршрут: кнопка створення й
рядок каталогу відкривають той самий `product-form` через `MatDialog.open()`, з тим самим
"Скасувати"/"Зберегти" знизу. Наслідок для `gallery/product-gallery-dialog.*` — компонент
лишається в межах підфічі `gallery/` (правило 15), але перестає бути окремим
`MatDialog`: він рендериться як секція нагорі `product-form`, а не відкривається кнопкою
з форми. Управління галереєю (вивантаження, головний кадр, видалення) не змінюється —
змінюється лише те, що ця секція вже на екрані, коли відкрито картку, а не за окремим
кліком.

**Поставка 2 додає:** `modules/ai/` (сервіс виклику моделі, адаптер Anthropic — розпізнає
товар із фото й окремо вміє текстовий виклик без фото для області `field`, [ADR 0014](adr/0014-let-ai-recognize-the-item-from-photos.md)/[ADR 0015](adr/0015-add-per-field-text-rewrite-scope.md)),
entity й репозиторій таблиці пропозицій усередині `products`, `src/queue.ts`,
`src/worker.ts`, `contracts/ai.contract.ts`, сервіс `worker` у `docker-compose.yml` і рядок
у `.dependency-cruiser.cjs` — перелік entity там **іменний**, тож нову треба дописати явно.
У `product-form/` додається панель з парними полями (поточне значення ліворуч, остання
пропозиція праворуч), кнопкою «Generate all» і кнопками «? -> AI» / «<- AI» на кожному
текстовому полі (T32) — деталі в §6, сценарій 10. Поле ціни отримує варіант тієї самої
панелі з однією кнопкою «? -> AI» і без «<- AI» (T32, PRD AC-23–AC-25) — асиметрія
пояснена в §4, уточнення 2026-09-13.

**Сім місць реєстрації**, де файл легко створити й забути підʼєднати:
...
| 3 | `.dependency-cruiser.cjs` | іменний перелік entity — у поставці 2 |
| 4 | `envSchema` у `src/config.ts` | креденшели R2 і публічний домен бакета |
| 5 | `.env.example` | ті самі змінні як зразок |
| 6 | `docker-compose.yml` | проброс змінних R2 у сервіс `api` |
| 7 | `infra/caddy/Caddyfile` | ліміт розміру тіла — парний до `config.http.bodyLimitBytes` |
...
результат є окремим записом (§4 S3). Сам запуск завершується `failed` з кодом
`price_unavailable`: статусу per-scope немає, а що бракує саме ціни, видно з відсутності
пропозиції `price` (рішення T24, data-model.md). Полінг стану в поставці 2 стає потрібним — генерація
триває десятки секунд; у поставці 1 його немає, бо фонової роботи немає теж.

**Сценарій 6 — ручне заповнення картки (US-06, AC-09, AC-12).**
...
sequenceDiagram
    ...
    else є хоча б один кадр
        api->>pg: перевіряє ліміт частоти запусків — запуск це прямі гроші (§8)
        api->>pg: ставить задачу з ключем ідемпотентності (картка, область, версія входу — хеш кадрів)
...
Область запуску має чотири значення — «тексти», «ціна», «обидва», «поле» (S7,
[ADR 0015](adr/0015-add-per-field-text-rewrite-scope.md)) — без «ціни» AC-10b не має
предмета, без «поля» немає регенерації одного поля з чернетки. Ключ ідемпотентності для
кожної області рахує сервер по-своєму (§4 S6/S7, [data-model.md](data-model.md) Open items).
...
sequenceDiagram
    ...
    Note over user,web: «? -> AI» біля ціни доступна будь-коли, поки є заголовок —<br/>не лише після часткової відмови (сценарій 5, AC-23)
    user->>web: натискає «? -> AI» біля ціни
    web->>api: запуск з областю «ціна»
    api->>pg: перевіряє titleProm/titleOlx картки
    alt немає ні titleProm, ні titleOlx
        api-->>web: preparation_input_incomplete, details.missing: [title] (AC-27)
        web-->>user: запуск не відбувся, картка лишається цілою
    else є titleProm або titleOlx
...
Область запуску дозволяє попросити саму ціну, не платячи вдруге за тексти, і кнопка більше
не привʼязана до відновлення після часткової відмови — user тисне її й на щойно відкритій
картці (AC-23). Форма результату тут інша, ніж у текстів, і це не дрібниця — пропозиція несе
діапазон із двох чисел, тоді як поле `price` картки одне (`decimal(12,2)`, `not null default
0`). Рішення, яке §4 (версія цього документа до 2026-09-13) лишало етапу 08: таблиця
пропозицій тримає діапазон як є (`{priceFrom, priceTo}`, [data-model.md](data-model.md)), а
до скаляра він доходить не автоматичним записом і не кліком, а звичайним ручним введенням
`user`-а (AC-25) — показаний діапазон лишається текстом-довідкою, не елементом керування.
`acceptFieldSuggestion` для `field: price` не викликається взагалі — відмовляє кодом
`price_suggestion_readonly` (AC-26; §4, уточнення 2026-09-13; §11).

**Вхід запиту (AC-08, AC-23, AC-27).** Заголовок і опис беруться незалежно один від одного,
кожен з перевагою Prom: `title = titleProm ?? titleOlx`, `description = descriptionProm ??
descriptionOlx`. Запит до `web_search` — `${title} ${description}`, коли опис є, інакше
просто `title`; сам по собі опис без заголовка входом не є — формула несиметрична навмисно,
бо заголовок завжди коротший і завжди присутній, коли картка взагалі має хоч щось написане
вручну чи згенероване (AC-05 пише обидва заголовки в тому самому запуску, що й описи). Гейт
перед постановкою задачі — той самий патерн коштом одного читання, що й AC-06 для кадрів
(сценарій 7): перевірка дешевша за запуск, а запуск коштує грошей.
...
**Сценарій 10 — поставка 2: регенерація одного поля з чернетки (US-10, AC-21, AC-22, §4 S7).**
...
sequenceDiagram
    ...
    user->>web: пише текст у полі сам (чернетка ще не збережена)
    user->>web: натискає «? -> AI» для цього поля
    alt чернетка порожня
        web-->>user: дія недоступна (AC-22)
    else є текст
        web->>api: запуск з областю field, field і draftText у тілі
        api->>pg: рахунок ліміту частоти — той самий, що для інших областей (PRD §6.1)
        api->>pg: ставить задачу з ключем ідемпотентності — хеш (field, draftText)
        api-->>web: запуск прийнято
        worker->>pg: бере задачу
    ...
    web->>api: приймає пропозицію поля (той самий acceptFieldSuggestion, що й для фото-запуску)
    api->>pg: пише значення в поле картки
    api-->>web: поле зліва оновлене значенням пропозиції
```

Ця область не додає нового ендпоінту прийняття: «<- AI» на будь-якому полі — це той самий
`acceptFieldSuggestion`, яким уже приймаються пропозиції з «Generate all». На щойно
створеній картці перша пропозиція для поля застосовується сама (ADR 0006, порожнє поле), і
кнопка «<- AI» просто нічого не змінює повторним кліком — активною вона стає саме тоді,
коли `user` уже правив поле руками і нова пропозиція розійшлася з полем (AC-11).

**Уточнення 2026-09-12 (макет картки).** Доки в галереї немає жодного кадру, усі текстові й
boolean поля, «Generate all» і «Зберегти» недоступні для взаємодії (AC-20) — власного
сценарію це не має, бо це стан форми на фронті, а не запит до `api`.
...
**Маркування в таблиці каталогу (AC-15, T22).** `isReady` уже їде в кожному рядку
`ProductList.items` (`contracts/openapi.yaml`) — новий запит бекенду не потрібен. Каталог
малює бейдж («Готово» / «Неготово», колір за станом) поруч із рядком; для неготової картки
підказка (`matTooltip`) називає, чого бракує — заголовок Prom, опис Prom, заголовок OLX,
опис OLX, ціна чи галерея. Перелік прогалин рахується **на фронті** з полів, які й так
приїхали в `Product` (`titleProm`, `descriptionProm`, `titleOlx`, `descriptionOlx`, `price`,
`images`), а не окремим полем відповіді: сервер уже віддає всі вихідні значення, і друге
джерело правди для того самого факту дублювало б `isReady`, а не додавало нового. Так само
малюється маркування у відкритій картці (`product-form`), а не лише в таблиці — AC-15
покриває обидва місця одним предикатом.

**Перегляд фото з каталогу (AC-40, T43).** Мініатюра й лічильник кадрів у рядку відкривають
діалог перегляду, а не форму картки. Власного сценарію він так само не має: кадри з
повними адресами вже приїхали в `ProductList.items`, і браузер бере їх з R2 напряму
([ADR 0007](adr/0007-serve-images-from-a-public-bucket.md)). Редагування галереї лишається у формі.
...
| **Обмеження частоти запусків підготовки** (поставка 2) | Окреме від наявного обмеження на спроби входу: запуски — це прямі гроші (PRD §6.1). 20 запусків на картку за годину, спільні для всіх областей (рішення T24). Коли ліміт вичерпано, `user` бачить це явно, а не отримує тишу | PRD §6.1 · `src/config.ts` |
...
| [0014](adr/0014-let-ai-recognize-the-item-from-photos.md) | Дозволити AI розпізнавати товар за фото, а не вимагати ручного опису заздалегідь | Accepted | §1, §4 S6 |
| [0015](adr/0015-add-per-field-text-rewrite-scope.md) | Регенерувати одне поле з його чернетки текстом — нова область запуску `field` | Accepted | §4 S7, §6 сценарій 10 |
| [0016](adr/0016-store-the-prom-description-as-html.md) | Зберігати опис для Prom як HTML і редагувати його міні-редактором | Accepted | §2, §4 S8 |
...
- **Кнопка ціни без «<- AI»** (§4 уточнення 2026-09-13, §6 сценарій 8) — асиметрія випливає
  прямо з типу даних (діапазон проти скаляра `price`), а не є окремим архітектурним вибором:
  ADR 0015 уже вирішив, що область `field` не застосовна до ціни, цей прохід лише домальовує
  UI для наявного `scope: price`.
- **Маркування готовності в таблиці каталогу** (§6, «Маркування в таблиці каталогу») —
  адитивний UI над уже спроектованим `isReady` ([ADR 0009](adr/0009-derive-card-readiness-instead-of-storing-it.md)),
  без нового поля відповіді й без нового запиту.
...
|---|---|---|---|
| ~~**Головна обіцянка PRD §2 не виконується першою поставкою.** «Тексти й ціна одним запуском» — це US-03, US-04 і US-08, які §1 переніс у поставку 2. Разом з ними втрачають предмет **три з чотирьох KPI PRD §7** (≤ 5 хв на картку, ≥ 50% прийнятих без правки, ≥ 80% без виходу в зовнішній чат): усі три міряють ефект від моделі й датовані «за 30 днів після релізу»~~ | ~~High~~ **Закрито 2026-09-07** | Внесено [T01](tasks/align-prd-with-architecture.md): три KPI PRD §7 датовані «за 30 днів після релізу **поставки 2**», а «готових карток на місяць» названо єдиним рядком, який має предмет уже після поставки 1. Рубіж розрізу задач той самий: [tracker.md](tasks/tracker.md) веде поставки 0, 1 і 2 окремими розділами | Serhii |
...
| ~~**Рядок `CLAUDE.md` «Не зберігаємо зображення в БД і не проксуємо їх через API — тільки R2 + presigned URL» містить три твердження, і чинні з них не всі.** «Не зберігаємо в БД» — чинне без застережень. «Не проксуємо через API» — чинне для **читання** (браузер іде в R2 напряму), але не для **запису**: кадр проходить через `api` за [ADR 0004](adr/0004-validate-uploads-in-api-before-r2.md), інакше перевірку до сховища виконати неможливо. «Тільки presigned URL» — перекрито [ADR 0007](adr/0007-serve-images-from-a-public-bucket.md)~~ | ~~Medium~~ **Закрито 2026-09-08** | ~~Переписати рядок розділу «Чого НЕ робимо» так, щоб він розрізняв читання й запис; ADR 0004 і ADR 0007 лишаються джерелами правди. Той самий рядок продубльовано в `ARCHITECTURE.md`, розділ «Межі, які тримаємо свідомо». **Due: до `break-tasks`** — єдиний з рядків цього строку, який лишається відкритим: його веде окрема story [T02](tasks/rewrite-r2-boundary-rule.md), бо правка чіпає кореневий `CLAUDE.md`, а не документи фічі~~ Правку внесено [T02](tasks/rewrite-r2-boundary-rule.md): рядок «Чого НЕ робимо» в `CLAUDE.md` і «Межі, які тримаємо свідомо» в `ARCHITECTURE.md` розрізняють читання й запис дослівно однаковим формулюванням і посилаються на ADR 0004 і ADR 0007 | Serhii |
| ~~**PRD розійшовся з архітектурою у трьох рядках:** §6.1 («посилання на кадр обмежене в часі») проти [ADR 0007](adr/0007-serve-images-from-a-public-bucket.md); AC-15 («user позначає картку готовою») проти [ADR 0009](adr/0009-derive-card-readiness-instead-of-storing-it.md); §6, рядок «Відгук на завантаження кадру» — джерелом виміру названо pino-логи, а превʼю малює браузер із локального файлу, тож у логах сервера цієї величини немає (§10 QG-2)~~ | ~~High~~ **Закрито 2026-09-07** | Внесено [T01](tasks/align-prd-with-architecture.md) одним комітом: §6.1 описує постійну адресу з публічного бакета й невгадуваність ключа замість строку придатності; AC-15 переписано на предикат при читанні, дії «позначити готовою» в ньому більше немає; рядок «Відгук на завантаження кадру» розкладено на дві величини — превʼю з секундоміром у браузері, а тривалість запиту з pino лишається спостереженням без порога | Serhii |
| ~~**PRD не має ні user story, ні критерію приймання на видалення**, хоча воно входить в обсяг ([ADR 0012](adr/0012-delete-permanently-in-the-same-request.md)) і без нього межа AC-02 у десять кадрів стає глухим кутом. Тією ж дірою бракує AC на відмову сховища при видаленні — AC-10 описує лише завантаження й підготовку~~ | ~~High~~ **Закрито 2026-09-07** | Внесено [T01](tasks/align-prd-with-architecture.md): PRD має US-09 і три AC — AC-16 (видалення кадру: обʼєкт, потім рядок), AC-17 (сховище недоступне — не видаляється нічого), AC-18 (видалення картки остаточне, разом з обʼєктами кадрів). Опис `deleteProduct` у [openapi.yaml](contracts/openapi.yaml) більше не називає прогалину, а посилається на ці номери — етап 15 будує з них тест-план | Serhii |
| ~~**PRD §6, рядок «3 повтори» описує рівень клієнта сховища, а не застосунку.** Формулювання писалося для черги, яка повторює задачу; після [ADR 0011](adr/0011-store-only-the-original-frame.md) черги в поставці 1 немає, і три спроби робить клієнт S3 (§6, сценарій 2)~~ | ~~Low~~ **Закрито 2026-09-07** | Внесено [T01](tasks/align-prd-with-architecture.md): рядок §6 називає три спроби роботою клієнта сховища, а абзац під таблицею й AC-10 додають, що власного циклу повторів застосунок не має і повтор є дією людини | Serhii |
| ~~**Два питання PRD §8 мають строк «до підписання PRD», а PRD стоїть Approved.** Перше — `SPEC.md` Goal 1 проти ручного розпізнавання; друге — «предмет з особистої колекції» на комерційному обсязі, що має правові наслідки для статусу продавця (idea-brief §10, §15) і йде саме в текст під OLX, який готує поставка 2~~ | ~~Medium~~ **Закрито 2026-09-21** | Перше закрито правкою `SPEC.md` (рядок вище). Друге закрив власник до прогону приймання поставки 2 ([T33](tasks/verify-delivery-2.md)): заяви не вимагаємо, промпт текстів її не містить | Serhii |
| ~~**Макет картки товару (2026-09-12) суперечив підписаному PRD.** idea-brief §13, PRD §3 non-goal і CONTEXT.md явно й свідомо фіксували ручне розпізнавання речі за фото (US-02); макет показав кнопку, яка розпізнає товар автоматично, а також новий per-field виклик «? -> AI», якого контракт не мав узагалі.~~ | ~~High~~ **Закрито 2026-09-12** | Внесено [ADR 0014](adr/0014-let-ai-recognize-the-item-from-photos.md) і [ADR 0015](adr/0015-add-per-field-text-rewrite-scope.md): US-02/AC-04 вилучені, US-03/AC-05/AC-06 переписані під фото-розпізнавання, додані US-10/AC-19…AC-22, нова область запуску `field`, `product_field_suggestions.field` розширено `title_prom`/`title_olx`. `product-form` стає `MatDialog` замість маршруту, галерея — вбудована секція замість окремого діалогу (§5). Правки внесені тим самим проходом у PRD, CONTEXT.md, data-model.md, openapi.yaml і задачах T14/T20/T21/T22/T24/T27-T29/T32 | Serhii |
| ~~**idea-brief §15 лишається незакритим:** вимога «безкоштовно або мінімальний cost» проти зафіксованої моделі `claude-opus-5` (§2). Строк цього питання — «до архітектурної фази», тобто цей прохід~~ | ~~Medium~~ **Закрито 2026-09-18** | ~~Прохід закрив половину: конфігурація вже заточена під ціну (три кадри, 1568 px, plain text), модель лишається константою, а облік `usage` входить в обсяг (AC-14). Числова стеля **не** ставиться. **Due: після першого місяця роботи поставки 2**~~ [ADR 0004](../../adr/0004-use-sonnet-5-for-card-preparation.md) замінив модель на `claude-sonnet-5` з `effort: low` на всіх викликах: оцінка ≈ $0.08–0.10 на картку, 100 карток у межах $10 на місяць. Замір — `usage` першого місяця; тригер перегляду й кроки повернення до сильнішої моделі — в ADR | Serhii |
...
| ~~**`FieldSuggestion.value` для `field: price` несумісний з generic `acceptFieldSuggestion`.** `openapi.yaml` сам позначав форму значення «best-effort, низька впевненість походження»: пропозиція несе діапазон `{priceFrom, priceTo}`, а `products.price` — скаляр `decimal(12,2)`; ендпоінт,описаний як «пише значення пропозиції в поле картки» generic-ом за `field` (T30), фізично не міг би записати діапазон у скаляр~~ | ~~Medium~~ **Закрито 2026-09-13** | Внесено разом з PRD AC-23–AC-26 (AC-26 фіксує саму відмову): `acceptFieldSuggestion` explicit-но виключає `field: price` (T30); показаний діапазон лишається текстом-довідкою, ціна потрапляє в поле лише ручним введенням user-а (§6, сценарій 8) і звичайним збереженням картки, а не через цей ендпоінт. `data-model.md` Open items і `openapi.yaml` `FieldSuggestion.value` більше не позначені TBD для `price` | Serhii |
...
Терміни домену не передруковуються сюди: вони живуть у
[кореневому CONTEXT.md](../../../CONTEXT.md) — слова рівня системи — і в
[CONTEXT.md цієї фічі](CONTEXT.md) — слова, які фіча вводить або звужує, разом з
інваріантами й доменними кодами помилок. Копія в цьому розділі розходилася б з ними
мовчки: вона вже встигла це зробити — «кадр» тут мав межу «NOT галерея», а у фічевому
CONTEXT.md «NOT похідний розмір», і читач залежно від файлу отримував половину.
