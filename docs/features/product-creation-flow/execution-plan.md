...
| 19f | T45 | Результат збереження | `tdd` | `pw`: успіх зі сповіщенням і помилка з `route` 422 | — | Поведінка під тест: закриття з `true`, текст сповіщення, помилка в рядку дій. Нових файлів немає, тож `tdd` не впреться в Gate 2. RED переписує два тести T39 про бейдж після збереження: це зміна вимоги, а не «лагодження» тестів. Крок 7 (`PRD.md`, примітка в T39) — руками |
