...
### `product_preparation_runs` — поставка 2, створено
...
| `error_code` | VARCHAR(64) | NULL | Доменний код при `failed` — той самий, що фронт мапить у текст (AC-10). `price_unavailable` — часткова відмова `both`: тексти записані, ціни немає (AC-10b). `model_unavailable` — вичерпано `retryLimit` задачі, пропозицій немає (AC-10, [events.md](contracts/events.md)) |
...
Створена міграцією `1789736913481-create-preparation-tables` ([T26](tasks/add-preparation-tables-migration.md)), після черги й `worker`
([T25](tasks/add-queue-and-worker.md)): таблиця без процесу, що в неї пише, не мала сенсу раніше.
...
- [x] **Вікно обмеження частоти запусків.** Закрито 2026-09-18: **20 запусків на картку за
  годину**, спільні для всіх областей (`texts`, `price`, `both`, `field`) — кожна з них є
  платним викликом моделі (PRD §6.1). Константа в `src/config.ts` ([T29](tasks/add-preparation-run-endpoints.md)),
  схему не змінює: вхід — `created_at` цієї ж таблиці.
- [x] **`status` при частковій відмові `scope: both`.** Закрито 2026-09-18: колонки чи таблиці
  статусу per-scope **не заводимо**. Запуск, у якому тексти записались, а ціна ні,
  завершується `status = failed` з `error_code = price_unavailable`; пропозиції текстів
  лишаються, бо кожна є окремим рядком. Що саме бракує, виводиться з даних: у запуску немає
  пропозиції з `field: price`. Ціну відновлює окремий запуск `scope: price` (AC-10b,
  sad.md сценарій 8). Окремий статус per-scope дублював би факт, який уже несе відсутність
  рядка пропозиції.
