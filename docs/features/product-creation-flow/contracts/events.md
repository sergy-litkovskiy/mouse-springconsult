...
`retryLimit + 1` спроб по `expireInSeconds`, паузи між ними (з `retryBackoff` кожна
наступна вдвічі довша) і запас на полінг та на сам період обходу — ≈16 хв за нинішніх
значень. Живий запуск обхід не чіпає (AC-40): молодший за поріг лишається як є, а
